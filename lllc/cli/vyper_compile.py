...
import lllc
from lllc.cli import vyper_json
from lllc.cli.utils import (
    extract_file_interface_imports,
    get_interface_file_path,
)
from lllc.compiler.settings import VYPER_TRACEBACK_LIMIT
from lllc.evm.opcodes import DEFAULT_EVM_VERSION, EVM_VERSIONS
from lllc.old_codegen import parser_utils
from lllc.typing import ContractCodes, ContractPath, OutputFormats
...
def _parse_args(argv):
    ...
    parser.add_argument(
        "--version", action="version", version=f"{lllc.__version__}+commit.{lllc.__commit__}",
    )
    parser.add_argument(
...
def compile_files(
    ...
    compiler_data = lllc.compile_codes(
        contract_sources,
        final_formats,
    ...
    if show_version:
        compiler_data["version"] = lllc.__version__
